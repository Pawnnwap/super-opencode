...
def show_task_form(*, job_manager, workspace: Path | None) -> None:
    if not workspace:
        st.warning("Set workspace path in Configuration first.")
        return
    ...
    with st.expander("Start New Task", expanded=no_existing_jobs):
        ...
        st.caption("Each task gets isolated workspace directory.")
...
def show_evo_setup_screen(*, job_manager, render_existing_protocol_banner) -> None:
    st.markdown(
        "Point supervisor + opencode at **this codebase itself**. "
        "Describe what you want improved or debugged - system will "
        "auto-generate `meta_protocol.md` from live source tree, "
        "then run full supervisor loop.",
    )
    ...
    render_existing_protocol_banner(
        ...
        reuse_label="Use existing meta_protocol.md",
...
def page_run(*, job_manager, pill_map: dict[str, str]) -> None:
    ...
    if job_id:
        ...
        JobStatusScreen(
            ...
            running_message="Task running. Safe close tab or refresh.",
            pill_map=pill_map,
        ).render()
        return
